...
public interface ChatEmpTokenRepo extends JpaRepository<ChatEmpToken, Integer> {
	...
	@Query(value = " SELECT e.emp_id, e.ex_var1 AS token, h.header_name  FROM emp_info e INNER JOIN t_chat_task_header h ON FIND_IN_SET( e.emp_id, CONCAT( h.created_user_id, ',', h.admin_user_ids, ',', h.assign_user_ids ) ) > 0 WHERE h.header_id=1  ", nativeQuery = true)
